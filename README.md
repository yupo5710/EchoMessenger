...
### 2. 예외 상황 처리 및 데이터 가공
* **어려웠던 점**: 사용자가 아무것도 입력하지 않고 전송을 누르거나, 공백만 입력했을 때 빈 줄이 추가되는 문제가 있었습니다. 또한 매우 긴 문장이 입력될 경우 리스트 박스의 가독성이 떨어졌습니다.
* **해결 방안**: 
    * `string.IsNullOrWhiteSpace()`를 사용하여 유효한 메시지만 전송되도록 방어 로직을 구축했습니다.
    * `Substring(0, 50)`을 적용하여 50자가 넘는 입력은 자동으로 제한하고 사용자에게 알림을 주어 데이터의 일관성을 유지했습니다.
    * `DateTime.Now`와 문자열 보간법(`$""`)을 사용해 메시지에 타임스탬프를 결합하는 실용적인 데이터 가공법을 익혔습니다.

### 3. 사용자 경험(UX) 고려
* **어려웠던 점**: 메시지를 보낸 후 입력창에 남은 텍스트를 일일이 지우고 다시 마우스로 클릭해야 하는 점이 불편하게 느껴졌습니다.
* **해결 방안**: `txtMessage.Clear();`와 `txtMessage.Focus();`를 전송 로직 끝에 배치하여, 전송 즉시 다음 입력을 준비할 수 있도록 흐름을 개선했습니다. 작은 코드 한 줄이 사용자의 편의성에 큰 영향을 미친다는 점을 깨달았습니다.
